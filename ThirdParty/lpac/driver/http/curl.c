...
static int http_interface_transmit(struct euicc_ctx *ctx, const char *url, uint32_t *rcode, uint8_t **rx,
    ...
    // Validate SM-DP+/SM-DS certificates by default. The developer override
    // is opt-in and is never enabled merely because this is a debug build.
    if (getenv("LPAC_HTTP_INSECURE")) {
