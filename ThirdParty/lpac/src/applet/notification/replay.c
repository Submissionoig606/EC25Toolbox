...
static int handle_notification(const uint32_t seqNumber, const struct es10b_pending_notification notification) {
    ...
    if (es9p_handle_notification(&euicc_ctx, notification.b64_PendingNotification)) {
        char detail[256];
        snprintf(detail, sizeof(detail), "%s; subjectIdentifier=%s; subjectCode=%s; reasonCode=%s",
                 euicc_ctx.http.status.message, euicc_ctx.http.status.subjectIdentifier,
                 euicc_ctx.http.status.subjectCode, euicc_ctx.http.status.reasonCode);
        jprint_error("es9p_handle_notification", detail);
        return -1;
    }
